...
figure_name = os.path.join(fig_dir, 'wprpFIG_M_106_110_BCRS_nt.png')
plt.figure(0, (4.5,4))
x, y2, y1 = np.loadtxt( os.path.join(BM_dir, 'wprp_M_106_110_shift_4_BC.txt'), unpack = True)
plt.plot(x, (y1/1e4+ y2/1e4)/2., color='darkblue', ls='dashed')
...
x, y2, y1 = np.loadtxt( os.path.join(BM_dir, 'wprp_M_106_110_shift_4_RS.txt'), unpack = True)
plt.plot(x, (y1/1e4+ y2/1e4)/2., color='darkred', ls='dashed')
plt.fill_between(x, y1/1e4, y2/1e4, color='darkred', alpha=0.5, label='Quiescent')
plt.xscale('log')
plt.yscale('log')
plt.xlabel(r"$r_p$ [$h^{-1}$Mpc]")
plt.ylabel(r"$w_p(r_p)$ [$h^{-1}$Mpc]")
...
###fig_dir = os.path.join(os.environ['GIT_STMOD'], 'data', 'validation', 'validation_GAL', 'WPRP')

###figure_name = os.path.join(fig_dir, 'wprp_nt.png')
###plt.figure(0, (4.5,4))
###x, y2, y1 = np.loadtxt( os.path.join(BM_dir, 'wprp_M_106_110_shift_4_BC.txt'), unpack = True)
###plt.plot(x, (y1/1e4+ y2/1e4)/2., color='darkblue', ls='dashed')
###plt.fill_between(x, y1/1e4, y2/1e4, color='darkblue', alpha=0.5, label='Star-forming')
