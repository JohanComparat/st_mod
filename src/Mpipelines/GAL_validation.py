...
validation_dir_Rband = os.path.join(validation_dir, 'RbandLuminosityFunction')
...
for meta in C_GAL.LC_MetaData[(enough_area)&(small_difference_minmax_1)&(small_difference_minmax_2)]:
    ...
    # SMF
    #
    os.system('mkdir -p ' + os.path.join( validation_dir_Stell, z_dir ))
    fig_out = os.path.join(validation_dir_Stell, z_dir, LC_dir+'_SMF_galaxies_replication_'+str(meta['jx'])+'_'+str(meta['jy'])+'_'+str(meta['jz']) +'.png' )
    plt.figure(1, (5., 5.))
    # literature SMF
    plt.plot(Msun_h70m2,  10**phi_All, lw=2 , label='Driver 2021, z=0') # +5*np.log10(h)
    #
    #
    ...
    if len(ilbert_sel.nonzero()[0])>0:
        ...
        plt.plot(mbins, fun(10**mbins)/(0.7**3), label=name, ls='dashed', lw=2)
    if z_mean<0.7:
        plt.errorbar( L15_M, y =  L15_phi*0.7**3, yerr=   L15_Err*0.7**3, label='Loveday 15, z<0.65')
    # literature DATA
    # COSMOS
    if z_mean<4:
        plt.hist(SM_arr_COSMOS, lw=1, weights=np.ones_like(SM_arr_COSMOS) / ( mdex * volume_COSMOS * completeness_COSMOS ) ,
                 bins = mbins, histtype = 'step', rasterized = True, label = 'COSMOS data')
    # GAMA G09
    if z_mean<0.6:
    ...
    # mock catalog
    plt.hist(MAG['log10_sm'], lw=2, weights=np.ones_like(MAG['log10_sm']) / ( mdex * volume_mock ) ,
             bins = mbins, histtype = 'step', rasterized = True, label = 'Mock', color='grey')
    ...
    # KLF
    #
    os.system('mkdir -p ' + os.path.join( validation_dir_Kband, z_dir ))
    fig_out = os.path.join(validation_dir_Kband, z_dir, LC_dir+'_KLF_galaxies_replication_'+str(meta['jx'])+'_'+str(meta['jy'])+'_'+str(meta['jz']) +'.png' )
    plt.figure(1, (5., 5.))
    ...
    # COSMOS
    if z_mean<4:
        plt.hist(MK_arr_COSMOS, lw=1, weights=np.ones_like(MK_arr_COSMOS) / ( mdex * volume_COSMOS * completeness_COSMOS ) ,
                 bins = kbins, histtype = 'step', rasterized = True, label = 'COSMOS data')
    # GAMA G09
    if z_mean<0.6:
        plt.hist(MK_arr_GAMA, lw=1, weights=np.ones_like(MK_arr_GAMA) / ( mdex * volume_GAMA * completeness_GAMA ) ,
                 bins = kbins, histtype = 'step', rasterized = True, label = 'GAMA data')
    # KIDS
    if z_mean<0.6:
        plt.hist(MK_arr_KIDS, lw=1, weights=np.ones_like(MK_arr_KIDS) / ( mdex * volume_KIDS * completeness_KIDS ) ,
                 bins = kbins, histtype = 'step', rasterized = True, label = 'KIDS data')
    # Mock catalog
    plt.hist(MAG['K_mag_abs'], lw=2, weights=np.ones_like(MAG['K_mag_abs']) / ( mdex * volume_mock ) ,
            bins = kbins, histtype = 'step', rasterized = True, label = 'Mock', color='grey')

    #
    plt.title(r'$\bar{z}$=' + str(np.round(z_mean, 3)))
    plt.ylabel(r'$\Phi/\; mag^{-1}\; Mpc^{-3}$')
    plt.xlabel('Absolute magnitude K')
    plt.yscale('log')
    plt.xlim(( -19, -26 ))
    plt.ylim((1e-6, 0.005))
    plt.legend(loc=0, fontsize=12)#, frameon=False)
    ...
    if z_mean<0.6:
        plt.hist(MR_arr_KIDS, lw=1, weights=np.ones_like(MR_arr_KIDS) / ( mdex * volume_KIDS * completeness_KIDS ) ,
                 bins = kbins, histtype = 'step', rasterized = True, label = 'KIDS data')
    # Mock catalog
    plt.hist(MAG['rmag'] - dm_itp(GAL['redshift_S']), lw=2, weights=np.ones_like(MAG['rmag']) / ( mdex * volume_mock ) ,
            bins = kbins, histtype = 'step', rasterized = True, label = 'Mock', color='grey')
    # SDSS
    #if z_mean<0.4:
    ...
    # Mock catalog
    plt.hist(MAG['rmag'], lw=2, weights=np.ones_like(MAG['rmag']) / (  meta['mean_area'] ) ,
