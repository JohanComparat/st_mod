...
validation_dir_WPRP = os.path.join(validation_dir, 'XCORR_SHELL')
...
for sky_tile in sky_map_hdu[(sky_map_hdu['OWNER']==2)|(sky_map_hdu['OWNER']==0)][::-1]:
	...
	dir_2pcf = os.path.join(esass_dir, 'XCORR_SHELLzlt04')
	os.system('mkdir -p ' + dir_2pcf)
	path_2_galaxy_file = os.path.join(os.environ['UCHUU'], LC_dir, str_field, 'glist.fits')
	path_2_agn_file = os.path.join(os.environ['UCHUU'], LC_dir, str_field, 'AGN_list_sigma_0.8_fsat_8.0.fits')
	path_2_agn_file_simput = os.path.join(os.environ['UCHUU'], LC_dir, str_field, 'AGN_list_sigma_0.8_fsat_8.0_simput.fits')
	path_2_gas_file = os.path.join(os.environ['UCHUU'], LC_dir, str_field, 'XGAS.fits')
	path_2_gas_file_simput = os.path.join(os.environ['UCHUU'], LC_dir, str_field, 'Xgas_bHS0.8_simput_final.fits')
	...
	basename = 'GALCEN_m' + str(np.round(11.0, 1)) + '_evAGNevCLUevBKG'
	p_2_2PCF = os.path.join(dir_2pcf, basename + '_CROSSCORR_05E20.wtheta.2pcf.fits')
	#  
	if os.path.isfile(path_2_galaxy_file) and os.path.isfile(path_2_simeventBKG_file) and os.path.isfile(path_2_simeventCLU_file) and os.path.isfile(path_2_simeventAGN_file) and not os.path.isfile(p_2_2PCF):
		GAL = Table.read(path_2_galaxy_file)
		GAL = GAL [ (GAL['upid'] == -1) ]

		t_in = Table.read(path_2_agn_file)
		print(len(t_in))
		XAGN = t_in[(t_in['FX_soft'] > -15.7) & (np.log10(t_in['obs_sm']) > 10) & (np.log10(t_in['obs_sm']) < 11.9) & (
					t_in['Mvir'] < 1e14)]
		XAGN_S = Table.read(path_2_agn_file_simput)
		agn_ids = np.unique(XAGN_S[XAGN['redshift_S']<0.4]['SRC_ID'])
		GAS = Table.read(path_2_gas_file)
		gas_ids = np.unique(GAS['id'][GAS['redshift_S']<0.4]).astype('int32')
		#GAS_S = Table.read(path_2_gas_file_simput)
		...
		evAGN = evAGN[(evAGN['SIGNAL']>=0.5)&(evAGN['SIGNAL']<=2.0)&(np.isin(evAGN['SRC_ID'].T[0].T,agn_ids))]
		evCLU = Table.read(path_2_simeventCLU_file)
		evCLU = evCLU[(evCLU['SIGNAL']>=0.5)&(evCLU['SIGNAL']<=2.0)&(np.isin(evCLU['SRC_ID'].T[0].T,gas_ids))]
		m0 = 10.0
		z1 = 0.18
		s10 = (np.log10(GAL['obs_sm'])>=m0) & (GAL['redshift_S']>0.05) & (GAL['redshift_S']<z1)
		basename = 'GALCEN_m'+str(np.round(m0,1))+'_evGAS'
		p_2_2PCF = os.path.join(dir_2pcf, basename +'_CROSSCORR_05E20.wtheta.2pcf.fits' )
...
for sky_tile in sky_map_hdu[(sky_map_hdu['OWNER']==2)|(sky_map_hdu['OWNER']==0)][::-1]:
	...
	path_2_simeventBKG_file = os.path.join(esass_dir, 'simBKGevt_'+str_field+'.fits')
	s_R = get_srvmap_rev(ra_fs, dec_fs, sky_tile)
	basename = 'GALSAT_m' + str(np.round(11.0, 1)) + '_evAGNevCLUevBKG'
	p_2_2PCF = os.path.join(dir_2pcf, basename + '_CROSSCORR_05E20.wtheta.2pcf.fits')
	#  
	if os.path.isfile(path_2_galaxy_file) and os.path.isfile(path_2_simeventBKG_file) and os.path.isfile(path_2_simeventCLU_file) and os.path.isfile(path_2_simeventAGN_file) and not os.path.isfile(p_2_2PCF):
		GAL = Table.read(path_2_galaxy_file)
		...
		z1 = 0.18
		s10 = (np.log10(GAL['obs_sm'])>=m0) & (GAL['redshift_S']>0.05) & (GAL['redshift_S']<z1)
		basename = 'GALSAT_m'+str(np.round(m0,1))+'_evGAS'
		p_2_2PCF = os.path.join(dir_2pcf, basename +'_CROSSCORR_05E20.wtheta.2pcf.fits' )
		if not os.path.isfile(p_2_2PCF):
			tabulate_XCORRwtheta_clustering_noW(evCLU['RA'], evCLU['DEC'],
											GAL['RA'][s10], GAL['DEC'][s10],
											ra_fs[s_R], dec_fs[s_R], p_2_2PCF)
		basename = 'GALSAT_m'+str(np.round(m0,1))+'_evBKG'
		p_2_2PCF = os.path.join(dir_2pcf, basename +'_CROSSCORR_05E20.wtheta.2pcf.fits' )
		if not os.path.isfile(p_2_2PCF):
			tabulate_XCORRwtheta_clustering_noW(evBKG['RA'], evBKG['DEC'],
											GAL['RA'][s10], GAL['DEC'][s10],
											ra_fs[s_R], dec_fs[s_R], p_2_2PCF)
		basename = 'GALSAT_m'+str(np.round(m0,1))+'_evAGN'
		p_2_2PCF = os.path.join(dir_2pcf, basename +'_CROSSCORR_05E20.wtheta.2pcf.fits' )
		if not os.path.isfile(p_2_2PCF):
			tabulate_XCORRwtheta_clustering_noW(evAGN['RA'], evAGN['DEC'],
											GAL['RA'][s10], GAL['DEC'][s10],
											ra_fs[s_R], dec_fs[s_R], p_2_2PCF)
		basename = 'GALSAT_m'+str(np.round(m0,1))+'_evAGNevCLU'
		p_2_2PCF = os.path.join(dir_2pcf, basename +'_CROSSCORR_05E20.wtheta.2pcf.fits' )
		if not os.path.isfile(p_2_2PCF):
			tabulate_XCORRwtheta_clustering_noW(np.hstack((evCLU['RA'], evAGN['RA'])), np.hstack((evCLU['DEC'],evAGN['DEC'])),
											GAL['RA'][s10], GAL['DEC'][s10],
											ra_fs[s_R], dec_fs[s_R], p_2_2PCF)
		basename = 'GALSAT_m'+str(np.round(m0,1))+'_evAGNevCLUevBKG'
		p_2_2PCF = os.path.join(dir_2pcf, basename +'_CROSSCORR_05E20.wtheta.2pcf.fits' )
		if not os.path.isfile(p_2_2PCF):
			tabulate_XCORRwtheta_clustering_noW(np.hstack((evCLU['RA'], evAGN['RA'], evBKG['RA'])), np.hstack((evCLU['DEC'], evAGN['DEC'], evBKG['DEC'])),
											GAL['RA'][s10], GAL['DEC'][s10],
