...
#test_predict_1_Ntimes = np.array([ gpr.predict(X[:1]) for ii in np.arange(100) ])
#test_sample_1_Ntimes = np.array([ gpr.sample_y(X[:1]) for ii in np.arange(100) ])


#t0 = time.time()
#test_predict_10 = gpr.predict(X[:1000])
#print((time.time()-t0)/1000)
#t0 = time.time()
#test_sample_10  = gpr.sample_y(X[:1000])
#print((time.time()-t0)/1000)

t0 = time.time()
y_all_pred = gpr.predict(X_all)#, return_std=True)
y_all = gpr.sample_y(X_all)#, return_std=True)
...
p2_out_COS = path_2_COSMOS[:-5]+'_COSMOStrainedGPmags_3.fits'
