...
for srv_val in sky_map_hdu['SRVMAP'][(sky_map_hdu['OWNER']==2)|(sky_map_hdu['OWNER']==0)]:
    ...
    all_tile_catalogues = np.array( glob.glob( os.path.join(os.environ['UCHUU'], LC_dir, str_field, 'z?p??', 'replication_*', 'glist.fits') ) )
    ...
    if len(all_tile_catalogues)>=1:
        ...
        merge_cat.write(os.path.join(os.environ['UCHUU'], LC_dir, str_field, 'glist.fits'), overwrite = True )
        print(os.path.join(os.environ['UCHUU'], LC_dir, str_field, 'glist.fits'), 'written, time spent=', time.time()-t0)
