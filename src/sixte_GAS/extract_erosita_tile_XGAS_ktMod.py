...
print(z_dir)
print('='*100)
p_2_catalogues = np.array( glob.glob( os.path.join(os.environ['UCHUU'], 'FullSky', z_dir, 'replication_*_*_*', basename+'.fits') ) )
p_2_catalogues.sort()
for p_2_catalogue in p_2_catalogues:
    ...
    if len(t_in)>0:
        ...
        for srv_val in U_srvmap_val:
            ...
            p_2_catalogue_out = os.path.join( dir_4_out, basename+'.fits')
