...
for sky_tile in sky_map_hdu[(sky_map_hdu['OWNER']==2)|(sky_map_hdu['OWNER']==0)]:
    ...
    # directory
    field_id = str_field
    indir = os.path.join("/home/idies/workspace/erosim/Uchuu/LCerass/", field_id, 's5_c030')
    outdir = os.path.join("/home/idies/workspace/erosim/Uchuu/LCerass/", field_id, 's5_eSASS')
    os.system('mkdir -p ' + outdir)
    # sb05_030108_020_FlareGTI_c030.fits  sb05_030108_020_Image_c030.fits.gz
    outprefix = field_id + "_"  # ""
    print(outdir)
    # input files
    all_EvtFiles = n.array(glob.glob(os.path.join(indir, '*_Image_c030.fits.gz')))
    print(all_EvtFiles)
    if len(all_EvtFiles)==0:
    #print(EvtFiles)
    #if (not os.path.isfile(EvtFiles)):
        print('Does not exist')
        continue
    else:
        EvtFiles = all_EvtFiles[0]
    print(EvtFiles)
    # SINGLE BAND
    # single band image
