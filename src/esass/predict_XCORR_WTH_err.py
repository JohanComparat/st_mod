...
#
# Companion of predict_XCORR_WTH.py: identical analysis, but the likelihood
# uses the formal per-bin uncertainties of the measurement rather than a flat
# 5 per cent. Outputs go to validation_GasGal/XCORR_ERR so the two runs can be
# compared side by side. See the SIGMA block below.
import os, sys
import matplotlib
matplotlib.use('Agg')
...
dir_fig = os.path.join( os.environ['GIT_STMOD_DATA'], 'data/validation/validation_GasGal/XCORR_ERR' )
...
#
# This file is predict_XCORR_WTH.py driven by the FORMAL per-bin uncertainties
# tabulated with the measurement (the wtheta_err column of Comparat et al. 2025)
# instead of a flat 5 per cent. Over the fitted range those are 0.01-1 per cent,
# so the likelihood is 5 to 500 times tighter here, bin by bin.
#
# Two caveats belong with any number this file produces.
#
#  1. wtheta_err is a per-bin uncertainty. It is NOT the covariance of the
#     measurement: a log-binned projected cross-correlation is strongly
#     correlated from bin to bin, and treating it as diagonal over-weights the
#     data. Using the formal errors therefore fixes the magnitude of the
#     assumed error while leaving its correlation structure wrong.
#  2. Because the errors are so small, chi2/dof measures whether the five-weight
#     mixture is the true model of the measurement, which it is not claimed to
#     be -- the templates are a finite mock realisation with its own shot noise,
#     which the likelihood does not model. A large chi2/dof here is expected and
#     is informative about model adequacy, not about a coding error.
# ---------------------------------------------------------------------------
SIGMA_TAG   = 'formal'
SIGMA_LABEL = 'formal per-bin uncertainties of Comparat et al. 2025 (wtheta_err)'

def SIGMA_FUN(y_fit, err_fit):
	return np.asarray(err_fit, dtype=float)
