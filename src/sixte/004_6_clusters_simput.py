...
logM500c_min = float(sys.argv[5]) # 13.0
logFX_min = float(sys.argv[6]) # 13.0
#option_group_corr = True
# initializes pathes to files

root_dir = os.path.join(os.environ[env])

#path_2_catalog = os.path.join(root_dir, env+'_eRO_CLU_b'+str(int(b_HS*10))+'_CM_'+cov_mat_option + "_pixS_" + str(pixel_size_image)+'_M500c_'+str(logM500c_min)+'_FX_'+str(logFX_min)+'_MGAS_Sept2021.fits')
#UNIT_fA1i_DIR_eRO_CLU_b8_CM_0_pixS_20.0_M500c_13.0_FX_-14.5_MGAS_Sept2021.fits
path_2_catalog = os.path.join(root_dir, env+'_eRO_CLU_b'+str(int(b_HS*10))+'_CM_'+cov_mat_option + "_pixS_" + str(pixel_size_image)+'_M500c_'+str(logM500c_min)+'_FX_'+str(logFX_min)+'_25Apr2022_Profiles')
dir_2_eRO_all = os.path.join(root_dir, 'cat_eRO_CLU_b'+str(int(b_HS*10))+'_CM_'+cov_mat_option + "_pixS_" + str(pixel_size_image)+'_M500c_'+str(logM500c_min)+'_FX_'+str(logFX_min) + '_galNH')
# dir_2_SMPT = os.path.join(os.environ[env], "cat_CLU_SIMPUT_b" + str(int(b_HS*10)) + '_CM_' + cov_mat_option + "_pixS_" + str(pixel_size_image)+'_M500c_'+str(logM500c_min)+'_FX_'+str(logFX_min) )
dir_2_SMPT = os.path.join(os.environ[env], "cat_CLU_SIMPUT_b" + str(int(b_HS*10)) + '_CM_' + cov_mat_option + "_pixS_" + str(pixel_size_image)+'_M500c_'+str(logM500c_min)+'_FX_'+str(logFX_min)+'_image_yes' )
...
s_1 = ( t_1['HALO_pid']==-1 ) & ( t_1['galaxy_SMHMR_mass']>10. ) & ( n.log10(t_1['HALO_Mvir']) > 13 ) & ( t_1['detectable'] )
t = t_1[s_1]
print(len(t_1), len(t))
...
#t['detectable'] = ( t['FX_soft_SIMPUT'] > 10**(t['flux_limit_eRASS8_pt']-0.5) )
#print('N total=', N_detec)
#print('density=', N_detec*n.pi/129600., '/deg2')

# detectability as a function of RA and DEC 
#N_detec = len(( t['FX_soft_SIMPUT'] > 10**(t['flux_limit_eRASS8_pt']-1) ).nonzero()[0])
#t['detectable'] = ( t['FX_soft_SIMPUT'] > 10**(t['flux_limit_eRASS8_pt']-1) )
print('N total=', N_detec)
print('density=', N_detec*n.pi/129600., '/deg2')
...
for HEALPIX_8_id in n.arange(healpy.nside2npix(8)):
	...
	if N_clu_all>0:
		...
		for jj, (id_min, id_max) in enumerate(zip(n.arange(0,N_clu_all,N_per_simput), n.arange(0,N_clu_all,N_per_simput)+N_per_simput)):
			...
			t_out.add_column(Column(name="FLUX",     dtype = n.float,    unit='erg/s/cm**2', data = t2['FX_soft_SIMPUT'][id_min:id_max])          )
