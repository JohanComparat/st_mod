...
if __name__ == '__main__':
    ...
    for sky_tile in sky_map_hdu[(sky_map_hdu['OWNER']==2)|(sky_map_hdu['OWNER']==0)] :
        ...
        simput_files = np.array([
            os.path.join(os.environ['UCHUU'], LC_dir, str_field, 'AGN_list_sigma_0.8_fsat_8.0_simput.fits'),
            ])
        print(sky_tile_id, simput_files)
        ra_cen = sky_tile['RA_CEN']
        dec_cen = sky_tile['DE_CEN']
        data_dir = os.path.join(os.environ['UCHUU'], LC_dir, str_field, erass_option + "_SEED_"+str(seed).zfill(3) +"_events_AGN_2025_04" )
